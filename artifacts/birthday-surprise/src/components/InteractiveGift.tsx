...
const noLines = [
  ...
  'The little kitty was really hoping for YES… 😭',
  'Aww… look at those tiny tears. 🥹💔',
  'Pleaseee… this kitty saved its biggest smile for YES. 🥺✨',
];
    ...
    <div className="pointer-events-none fixed inset-0 z-[300] overflow-hidden" aria-hidden="true">
      {Array.from({ length: 12 }, (_, i) => (
        <motion.span
          ...
          animate={{ opacity: [0, 1, 0], y: -260 - (i % 4) * 55, x: (i - 5.5) * 38, scale: [0.4, 1, 0.6] }}
          transition={{ duration: 2.1, delay: i * 0.035, ease: 'easeOut' }}
          className="absolute bottom-[28%] left-1/2 text-xl"
        >
          {i % 3 === 0 ? '💛' : i % 3 === 1 ? '✨' : '💖'}
        </motion.span>
      ))}
...
function GiftBox({ step, onTap, onFinished }: { step: number; onTap: () => void; onFinished: () => void }) {
  ...
  useEffect(() => {
    if (!opened) return;
    const timer = window.setTimeout(onFinished, 1750);
    return () => window.clearTimeout(timer);
  }, [opened, onFinished]);

  return (
    <div className="relative h-[390px] w-[300px] select-none transform-gpu sm:h-[450px] sm:w-[370px]">
      {/* Lightweight glow: opacity only, so mobile GPUs do less work. */}
      <motion.div
        animate={opened ? { opacity: [0.35, 0.65, 0] } : { opacity: [0.2, 0.38, 0.2] }}
        transition={{ duration: opened ? 1.5 : 2, repeat: opened ? 0 : Infinity, ease: 'easeInOut' }}
        className="absolute bottom-8 left-1/2 h-20 w-56 -translate-x-1/2 rounded-full bg-[#ffd76a]/45 blur-xl"
      />

      {/* Photo reveal: one GPU-friendly transform + opacity animation. */}
      <AnimatePresence>
        {opened && (
          <motion.div
            initial={{ opacity: 0, y: 72, scale: 0.72 }}
            animate={{ opacity: 1, y: -18, scale: 1 }}
            transition={{ duration: 1.45, ease: [0.22, 0.9, 0.32, 1] }}
      ...
      {/* Box body fades down gently after the photo has started rising. */}
      <motion.div
        animate={opened ? { opacity: 0, y: 16, scale: 0.98 } : { opacity: 1, y: 0, scale: 1 }}
        transition={{ duration: 1.35, delay: 0.12, ease: [0.22, 0.9, 0.32, 1] }}
      ...
      <AnimatePresence>
        {step < 3 && (
          <motion.button
            ...
            animate={{ opacity: 1, scale: step === 2 ? [1, 1.05, 1] : 1 }}
      ...
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-white/85 px-5 py-2 font-sans text-xs font-bold text-[#7c3d58] shadow-md">
        {step === 0 ? '1 / 3  •  Tap the lid' : step === 1 ? '2 / 3  •  Tap the ribbon' : step === 2 ? '3 / 3  •  Open the surprise' : 'Opening your memory… 💛'}
      </div>
    </div>
  );
}
...
function CatButton({ state, text }: { state: 'sad' | 'happy'; text: string }) {
  ...
  return (
    <motion.div
      ...
      className="pointer-events-none absolute -top-[142px] left-1/2 z-30 w-[230px] -translate-x-1/2 sm:-top-[158px] sm:w-[250px]"
    >
      <div className="relative mx-auto h-[128px] w-[100px] sm:h-[145px] sm:w-[112px]">
        <motion.img
          src={catIdle}
          alt="3D kitten from the reference image"
          loading="eager"
          decoding="async"
          draggable={false}
          className="h-full w-full object-contain drop-shadow-[0_10px_14px_rgba(40,10,28,.22)]"
          animate={sad ? { x: [0, -2, 2, 0], rotate: [0, -1.5, 1.5, 0] } : { y: [0, -5, 0], rotate: [-1, 1, -1, 0] }}
          transition={{ duration: sad ? 0.7 : 1, repeat: Infinity, ease: 'easeInOut' }}
        />

        {sad ? (
          <>
            <motion.span animate={{ y: [0, 15], opacity: [0, 1, 0] }} transition={{ duration: 0.75, repeat: Infinity }} className="absolute left-[28%] top-[35%] h-6 w-1.5 rounded-full bg-sky-300" />
            <motion.span animate={{ y: [0, 17], opacity: [0, 1, 0] }} transition={{ duration: 0.75, delay: 0.18, repeat: Infinity }} className="absolute right-[28%] top-[35%] h-6 w-1.5 rounded-full bg-sky-300" />
          </>
        ) : (
          <motion.div animate={{ scale: [1, 1.12, 1] }} transition={{ duration: 0.7, repeat: Infinity }} className="absolute left-1/2 top-[42%] h-3 w-7 -translate-x-1/2 rounded-b-full border-b-[3px] border-[#8b2d52]" />
        )}
      </div>
      <div className={`mx-auto -mt-1 w-fit max-w-[205px] rounded-2xl border px-3 py-2 text-center text-[11px] font-bold leading-tight shadow-md ${sad ? 'border-[#f3b5c7] bg-[#fff0f5] text-[#863b59]' : 'border-[#ffe38a] bg-[#fff8db] text-[#8a5b16]'}`}>
        {text}
      </div>
    </motion.div>
  );
    ...
    <section className="relative min-h-[100dvh] overflow-hidden bg-[radial-gradient(circle_at_20%_0%,#fff1c8_0%,transparent_28%),radial-gradient(circle_at_90%_40%,#f6c8d8_0%,transparent_30%),linear-gradient(145deg,#fffaf0,#f8e8ed_52%,#f6e0b8)] px-4 py-12 sm:px-6 sm:py-16">
      ...
      <div className="mx-auto w-full max-w-5xl">
        {!revealed ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.55 }} className="mx-auto max-w-4xl">
            <div className="relative overflow-hidden rounded-[34px] border-[5px] border-white/80 bg-[#f7eadb] p-2 shadow-[0_28px_80px_rgba(71,24,48,.2)] sm:rounded-[44px] sm:p-3">
            ...
            <div className="mt-7 rounded-[34px] border border-white/80 bg-white/65 px-5 py-8 text-center shadow-[0_26px_70px_rgba(70,25,50,.14)] sm:px-9 sm:py-10">
              ...
              <div className="relative mx-auto mt-9 flex max-w-md items-center justify-center gap-4 pt-[150px] sm:gap-6 sm:pt-[165px]">
                <CatButton state={answer === 'yes' ? 'happy' : 'sad'} text={answer === 'yes' ? 'YAY! The kitty is smiling with you! 🥹💛' : noCount === 0 ? 'Your little kitty is waiting for your choice… 🐾💛' : noLines[Math.min(noCount - 1, noLines.length - 1)]} />
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  animate={noCount > 0 ? { x: [0, -7, 7, -4, 4, 0] } : { x: 0 }}
                  transition={{ duration: 0.42, ease: 'easeInOut' }}
                  className="relative z-20 min-w-[132px] rounded-full border border-white bg-[#fff8fc] px-8 py-4 font-sans text-sm font-black tracking-[.12em] text-[#793e59] shadow-[0_14px_30px_rgba(83,35,58,.14)] sm:min-w-[150px] sm:text-base"
                >
                  NO
                </motion.button>
              </div>
              ...
              <AnimatePresence mode="wait">
                {answer === 'yes' ? (
                  <motion.div key="yes" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="mx-auto mt-7 max-w-2xl rounded-[26px] border border-[#f4d77c] bg-[linear-gradient(135deg,#fffdf3,#fff4cf)] px-5 py-6 shadow-lg sm:px-8">
                  ...
                  <motion.p key={noCount} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-5 font-sans text-xs font-semibold text-[#8b6170]">{noCount === 0 ? 'No pressure. Just a little question wrapped in a lot of love. 🌷' : 'The kitty is still hoping you’ll give YES a chance… 🥺💛'}</motion.p>
                )}
              </AnimatePresence>
            </div>
          </motion.div>
