      ...
      <AnimatePresence>
        {!entered && (
          <motion.div className="entrance-screen" initial={{ opacity: 1 }} exit={{ opacity: 0, scale: 1.03 }} transition={{ duration: 1 }}>
            ...
            <div className="entrance-sparkles">✦　·　♡　·　✧　·　♡　·　✦</div>
            <motion.button className="entrance-polaroid" whileHover={{ y: -8, rotate: -2 }} whileTap={{ scale: .97 }} onClick={() => setPhoto(0)}>
              <img src={photos[0]} alt="Bhavanika" />
              <span>tap me ♡</span>
            </motion.button>
            <p className="eyebrow">a little place made only for you</p>
            <h1>For <em>Bhavanika.</em></h1>
            <div className="entrance-copy">
              <strong>Wait… don't rush this part.</strong>
              <span>I made a tiny world for you.<br />Come in slowly and find the little things I hid for you.</span>
            </div>
            <button className="enter-button" onClick={() => { setEntered(true); window.setTimeout(startAmbient, 120); }}>
              <Heart size={16} fill="currentColor" /> come in, pookie
            </button>
        ...
        <>
          ...
          <header className="floating-nav">
            ...
            <button className="mini-player" onClick={() => document.getElementById('songs')?.scrollIntoView({ behavior: 'smooth' })}>
              <span className={ambientMuted ? 'equalizer' : 'equalizer is-playing'}><i /><i /><i /></span><span>Blue · background</span>
            </button>
          </header>

          <main>
            <section id="home" className="hero-chapter">
              <div className="hero-sun" /><div className="hero-petal-layer" />
              <motion.div className="hero-photo hero-photo-a" animate={{ y: [0, -12, 0], rotate: [3, 1, 3] }} transition={{ duration: 7, repeat: Infinity }} onClick={() => setPhoto(0)}><img src={photos[0]} alt="A memory of us" /><span>the beginning</span></motion.div>
              <motion.div className="hero-photo hero-photo-b" animate={{ y: [0, 14, 0], rotate: [-4, -2, -4] }} transition={{ duration: 8, repeat: Infinity }} onClick={() => setPhoto(5)}><img src={photos[5]} alt="Another memory" /><span>still us</span></motion.div>
              <div className="hero-content">
                ...
                <p className="hero-sub"><TypeLine text="I could have written you another paragraph. Instead, I wanted to give you a place that feels a little like us." /></p>
                <div className="hero-buttons"><a className="scroll-cue primary" href="#story">start our story <ArrowDown size={15} /></a><button className="ghost-pill" onClick={togglePlay}>{playing ? <Pause size={15} /> : <Play size={15} />} {playing ? 'pause my song' : 'play my song'}</button></div>
              </div>
              <div className="hero-orbit orbit-one" /><div className="hero-orbit orbit-two" />
            </section>

            <section id="story" className="chapter story-chapter">
              <div className="section-kicker">the part where everything changed</div>
              <div className="story-grid">
                <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .3 }}>
                  <p className="micro-label">I still think about this sometimes</p>
                  <h3>It was only<br /><em>a game…</em><br />until it wasn't.</h3>
                  <p className="lead">I logged into ZEPETO expecting nothing more than a game. Then I met you. We talked, joked, stayed a little longer, came back again… and somehow you became someone I started looking for even before I realised I was doing it.</p>
                  <div className="secret-note"><Sparkles size={16} /><span>I am ridiculously glad that one random game led me to you.</span></div>
                </motion.div>
                <motion.button className="story-photo-card" whileHover={{ scale: 1.02, rotate: -1 }} onClick={() => setPhoto(0)}><img src={photos[0]} alt="The beginning" /><span>tap the memory</span><div className="photo-caption">the beginning of my favourite chapter</div></motion.button>
              </div>
              <div className="distance-line"><span>you</span><div><i /><i /><i /><i /><i /></div><span>me</span><strong>different screens · somehow still so close</strong></div>
            </section>

            <section id="about" className="chapter about-chapter">
              <div className="section-kicker">if I had to explain why I fell</div>
              <div className="about-heading"><div><h3>Six little<br /><em>confessions.</em></h3><p>Don't just read them. Tap one. I hid the real answer underneath.</p></div><div className="about-stamp"><Heart size={18} fill="currentColor" /><span>made<br />for you</span></div></div>
              <div className="reasons-grid">{reasons.map(([title, copy], i) => <motion.button key={title} className={`reason-card ${reason === i ? 'is-open' : ''}`} whileTap={{ scale: .98 }} onClick={() => setReason(reason === i ? null : i)}><span className="reason-icon">{reason === i ? '♡' : '✦'}</span><strong>{title}</strong><i>{reason === i ? 'hide' : 'tap to read'}</i><AnimatePresence>{reason === i && <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }}>{copy}</motion.p>}</AnimatePresence></motion.button>)}</div>
              <div className="nickname-strip"><span>a few names I keep for you</span><b>thangoww</b><i>♡</i><b>en aval</b><i>♡</i><b>bujji kuttyy</b><i>♡</i><b>pookie</b></div>
            </section>

            <section id="memories" className="chapter memory-chapter">
              <div className="section-kicker">little pieces I never want to lose</div>
              <div className="memory-heading"><h3>A photo album<br /><em>you can touch.</em></h3><p>Tap a memory to open it. On desktop you can hover to peek; on mobile, every card is built for touch.</p></div>
              <div className="photo-mosaic">{photos.map((src, i) => <motion.button key={src} className={`memory-card memory-${i + 1}`} whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 35 }} viewport={{ once: true, amount: .1 }} whileHover={{ y: -9 }} onClick={() => setPhoto(i)}><div className="memory-inner"><div className="memory-front"><img src={src} alt={`Memory ${i + 1}`} loading="lazy" /><span>{String(i + 1).padStart(2, '0')}</span><small>tap to open</small></div><div className="memory-back"><Sparkles size={16} /><strong>{stories[i]}</strong><small>open the full memory</small></div></div></motion.button>)}</div>
            </section>

            <section className="chapter reel-chapter">
              <div className="reel-copy"><div className="section-kicker">a memory that keeps moving</div><h3>Let the pictures<br /><em>say it for me.</em></h3><p className="lead">This one moves because memories do too. Tap the arrows, tap the picture, or just let it drift.</p><div className="reel-controls"><button onClick={() => setReel(v => (v - 1 + photos.length) % photos.length)}><ChevronLeft /></button><div><span>{String(reel + 1).padStart(2, '0')}</span> / 14</div><button onClick={() => setReel(v => (v + 1) % photos.length)}><ChevronRight /></button></div></div>
              <motion.button className="reel-stage" whileHover={{ scale: 1.015 }} onClick={() => setPhoto(reel)}><AnimatePresence mode="wait"><motion.img key={reel} src={photos[reel]} alt="Moving memory" initial={{ opacity: 0, scale: 1.04 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .98 }} transition={{ duration: .65 }} /></AnimatePresence><div className="reel-overlay"><span>memory {String(reel + 1).padStart(2, '0')}</span><strong>{stories[reel]}</strong></div></motion.button>
            </section>
            ...
            <section id="timeline" className="chapter timeline-chapter">
              <div className="section-kicker">the little timeline of us</div>
